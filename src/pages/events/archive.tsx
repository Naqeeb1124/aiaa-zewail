            ...
            <main className="max-w-7xl mx-auto px-6 py-16 md:py-32">
                ...
                <div className="flex justify-center gap-2 mb-16 flex-wrap">
                    {['all', 'workshop', 'ceremony', 'webinar', 'social', 'competition'].map(type => (
                        <button
                            ...
                            className={`px-6 py-2 text-[10px] font-black uppercase tracking-widest transition-all ${filter === type
                                ? 'bg-deep text-white'
                                : 'bg-white text-ink-muted hover:text-deep border border-line'
                                }`}
                        >
                            {type}
                        </button>
                ...
                {loading ? (
                    <div className="flex justify-center py-20">
                        <div className="animate-spin h-12 w-12 border-b-2 border-deep"></div>
                    </div>
                ) : filteredEvents.length === 0 ? (
                    <div className="text-center py-24 bg-white border border-dashed border-line">
                        <p className="text-ink-muted font-black uppercase tracking-widest text-xs">No historical records match your search.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 gap-12">
                        {filteredEvents.map((event, index) => (
                            <div
                                key={event.id}
                                className="bg-white border border-slate-100px] overflow-hidden flex flex-col md:flex-row  transition-all duration-500 group"
                            >
                                {/* Image */}
                                <div className="md:w-1/3 bg-canvas h-64 md:h-auto relative overflow-hidden">
                                    <Image
                                    ...
                                    />
                                    <div className="absolute top-6 left-6 px-3 py-1 bg-white/90 backdrop-blur text-[8px] font-black text-deep uppercase tracking-widest border border-line">
                                        {event.category}
                                    </div>
                                </div>
                                ...
                                <div className="p-10 md:p-16 md:w-2/3 flex flex-col justify-between">
                                    <div>
                                        <div className="flex justify-between items-start mb-6">
                                            <div>
                                                <p className="text-growth text-[10px] font-black uppercase tracking-widest mb-2 flex items-center gap-2">
                                                ...
                                                <h3 className="text-3xl font-black text-ink group-hover:text-deep transition-colors uppercase tracking-tight">{event.title}</h3>
                                            </div>
                                            <span className="px-3 py-1 bg-accent-orange-soft text-ember text-[10px] font-black uppercase tracking-widest border border-accent-orange-soft">
                                                Archived
                                            </span>
                                        </div>
                                        <p className="text-ink-soft mb-8 leading-relaxed font-medium text-base line-clamp-4">{event.description}</p>
                                    </div>
                                    
                                    {event.resources && event.resources.length > 0 && (
                                        <div className="flex flex-wrap gap-3 pt-8 border-t border-canvas">
                                            {event.resources.map((res, i) => (
                                                <a
                                                    ...
                                                    className="flex items-center gap-2 px-5 py-2.5 bg-canvas border border-line text-[10px] font-black uppercase tracking-widest text-ink-soft hover:border-deep hover:text-deep transition-all"
                                                >
                                                    <span>
                                                        {res.type === 'slides' ? '📑' :
                                                            res.type === 'video' ? '▶️' :
